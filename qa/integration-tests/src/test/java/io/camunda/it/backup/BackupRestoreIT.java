...
public class BackupRestoreIT {
  ...
  @AutoClose private GenericContainer<?> searchContainer;

  @AutoClose private RestClient restClient;
          ...
          case ELASTICSEARCH -> {
            final var container =
                TestSearchContainers.createDefeaultElasticsearchContainer()
                    ...
                    .withEnv("path.repo", "/home");
            container.addFileSystemBind(
                repositoryDir.toString(), "/home", BindMode.READ_WRITE, SelinuxContext.SHARED);
            container.start();
  ...
  private void deleteAllIndices() throws IOException {
    final var indices =
        Stream.of("operate*", "tasklist*", "optimize*").map(i -> INDEX_PREFIX + "_" + i).toList();
    switch (config.databaseType) {
      case ELASTICSEARCH -> {
        esClient
            .indices()
            .delete(
                DeleteIndexRequest.of(b -> b.index(indices).expandWildcards(ExpandWildcard.All)));
      }
      default -> {
